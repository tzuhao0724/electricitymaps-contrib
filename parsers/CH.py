...
class extract_data(paeras_example):
    def get_solar_capacity_at(self,date: datetime) -> float:
    ...
    def fetch_swiss_exchanges(self,session, target_datetime, logger):
    ...
    def fetch_swiss_consumption(self,
    ...
    def fetch_production(self,
        ...
        exchanges = self.fetch_swiss_exchanges(r, now, logger)
        consumptions = self.fetch_swiss_consumption(r, now, logger)
        ...
        for p in productions:
            p["capacity"] = {
                "solar": self.get_solar_capacity_at(p["datetime"]),
            }

...
class extract_data(paeras_example):
    ...
    def _parse_ieso_hour(self,output, target_dt):
    ...
    def fetch_production(self,
        ...
        dt, xml = self._fetch_ieso_xml(target_datetime, session, logger, PRODUCTION_URL)
        ...
        all_productions = (
            {
                ...
                "dt": self._parse_ieso_hour(output, dt),
    ...
    def fetch_price(self,
        ...
        dt, xml = self._fetch_ieso_xml(target_datetime, session, logger, PRICE_URL)
        ...
        data = [
            {
                "datetime": self._parse_ieso_hour(price, dt),
    ...
    def fetch_exchange(self,
        ...
        dt, xml = self._fetch_ieso_xml(target_datetime, session, logger, EXCHANGES_URL)
...
if __name__ == "__main__":
    ...
    print(s.fetch_price(target_datetime=now.replace(hour=2)))

    print("we expect results for 2 months ago")
    print("fetch_price(target_datetime=now.shift(months=-2).datetime)) ->")
    print(s.fetch_price(target_datetime=now.shift(months=-2).datetime))

    print("there are likely no results for 2 years ago")
    print("fetch_price(target_datetime=now.shift(years=-2).datetime)) ->")
    print(s.fetch_price(target_datetime=now.shift(years=-2).datetime))

    print('fetch_exchange("CA-ON", "US-NY") ->')
    print(s.fetch_exchange("CA-ON", "US-NY"))

    print('fetch_exchange("CA-ON", "CA-QC", target_datetime=now.datetime)) ->')
    print(s.fetch_exchange("CA-ON", "CA-QC", target_datetime=now.datetime))
    ...
    print(s.fetch_exchange("CA-ON", "CA-MB", target_datetime=now.datetime))
    ...
    print(s.fetch_exchange("CA-ON", "CA-QC", target_datetime=now.replace(hour=2)))
    ...
    print(
        s.fetch_exchange("CA-ON", "CA-QC", target_datetime=now.shift(months=-2).datetime)
    )
    ...
    print(
        s.fetch_exchange("CA-ON", "CA-QC", target_datetime=now.shift(years=-2).datetime)
    )

    print("requesting an exchange with Nova Scotia should raise exception")
    print('fetch_exchange("CA-ON", "CA-NS")) ->')
    print(s.fetch_exchange("CA-ON", "CA-NS"))

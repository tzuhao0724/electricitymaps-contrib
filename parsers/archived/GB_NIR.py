...
class extract_data(paeras_example):
    def get_data(self,url, target_datetime, session=None):
        """
    ...
    def add_default_tz(self,timestamp):
    ...
    def create_production_df(self,text_data):
        """
    ...
    def create_exchange_df(self,text_data):
        """
    ...
    def production_processor(self,df) -> list:
        """Creates quarter hour datapoints for thermal production."""

        datapoints = []
        for index, row in df.iterrows():
            snapshot = {}
            snapshot["datetime"] = self.add_default_tz(
    ...
    def moyle_processor(self,df) -> list:
    ...
    def fetch_production(self,
        ...
        production_data = self.get_data(production_url, target_datetime)
        production_df = self.create_production_df(production_data)
        production = self.production_processor(production_df)
    ...
    def fetch_exchange(self,
        ...
        exchange_data = self.get_data(exchange_url, target_datetime)
        exchange_dataframe = self.create_exchange_df(exchange_data)
        if "->".join(sorted([zone_key1, zone_key2])) == "GB->GB-NIR":
            moyle = self.moyle_processor(exchange_dataframe)
            return moyle
        elif "->".join(sorted([zone_key1, zone_key2])) == "GB-NIR->IE":
            IE = self.IE_processor(exchange_dataframe)
            return IE
        else:
...
if __name__ == "__main__":
    ...
    print(s.fetch_exchange("GB-NIR", "GB"))
    print("fetch_exchange(GB-NIR, IE) ->")
    print(s.fetch_exchange("GB-NIR", "IE"))

...
class extract_data(paeras_example):
    def split_and_sum(self,expression) -> float:
        ...
        for row in rows:
            ...
            if len(elements) > 3:  # will find production rows
                ...
                if v1 == "GujaratCatered":
                    values["total consumption"] = self.split_and_sum(v2.split("MW")[0])
            elif len(elements) == 1:
    ...
    def fetch_production(self,
        ...
        value_map = self.fetch_data(zone_key, session, logger=logger)
...
if __name__ == "__main__":
    session = Session()
    s = extract_data()
    print(s.fetch_production("IN-GJ", session))
    print(s.fetch_consumption("IN-GJ", session))

...
class extract_data(paeras_example):
    def fetch_data(seelf,zone_key="IN-UP", session=None):
    ...
    def fetch_production(self,zone_key, session=None, target_datetime=None, logger=None) -> dict:
        """Method to get production data of Uttar Pradesh."""
        if target_datetime:
            raise NotImplementedError("This parser is not yet able to parse past dates")

        value_map = self.fetch_data(zone_key, session)
...
if __name__ == "__main__":
    session = Session()
    s = extract_data()
    print(s.fetch_production("IN-UP", session))
    print(s.fetch_consumption("IN-UP", session))

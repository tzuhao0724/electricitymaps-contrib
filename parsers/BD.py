...
class extract_data(paeras_example):
    ...
    def production_processer(self,df, target_datetime, old_format=False) -> list:
        """
        ...
        for index, row in df.iterrows():
            ...
            dt = self.timestamp_converter(index, target_datetime)
    ...
    def exchange_processer(self,df, target_datetime, old_format=False) -> list:
        """
    ...
    def excel_handler(self,shifted_target_datetime, logger: Logger) -> tuple:
        """
        ...
        if OLD_FORMAT:
            ...
            df = self.old_format_converter(df)
        else:
            if XLS_END:
                df = pd.read_excel(
                    URL,
                ...
                df = self.new_format_converter(df, logger)
            else:
    ...
    def fetch_production(self,
        ...
        df, OLD_FORMAT = self.excel_handler(shifted_target_datetime, logger)

        generation = self.production_processer(df, target_datetime, old_format=OLD_FORMAT)
    ...
    def fetch_exchange(self,
        ...
        df, OLD_FORMAT = self.excel_handler(shifted_target_datetime, logger)
        exchange = self.exchange_processer(df, target_datetime, old_format=OLD_FORMAT)
...
if __name__ == "__main__":
    ...
    print(s.fetch_production(target_datetime="20190110"))
    print("fetch_production(target_datetime=20180113)")
    print(s.fetch_production(target_datetime="20180113"))
    print("fetch_production(target_datetime=20170110)")
    print(s.fetch_production(target_datetime="20170110"))
    print("fetch_production(target_datetime=20150601)")
    print(s.fetch_production(target_datetime="20150601"))
    print("fetch_exchange(target_datetime=20190109)")
    print(s.fetch_production("BD", "IN", target_datetime="20190109"))
    print("fetch_exchange(target_datetime=20170109)")
    print(s.fetch_production("BD", "IN", target_datetime="20170109"))
    print("fetch_exchange(target_datetime=20150109)")
    print(s.fetch_production("BD", "IN", target_datetime="20150109"))
